# Step 4 load for dim_sources -- same shape as load_dim_peptides.py, just a
# different processed CSV and table name.
#
# Run from the repo root: python -m src.load.load_dim_sources
...
TABLE = "dim_sources"
COLUMNS = ["source_id", "source_name", "reliability"]


def extract():

    path = PROCESSED_DIR / "dimensions" / "sources.csv"
    return pd.read_csv(path)


def validate(df):

    assert_no_nulls(df, ["source_id", "source_name", "reliability"], TABLE)
